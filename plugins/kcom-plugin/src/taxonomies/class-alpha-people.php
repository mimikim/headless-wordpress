...
class Alpha_People extends Taxonomies {
	...
	public function add_default_terms() {
		$taxonomy = get_terms( 'alpha_people', array( 'hide_empty' => false ) );

		if ( empty( $taxonomy ) ) {
			$predefined_terms = $this->return_taxonomy_terms();

			foreach ( $predefined_terms as $term ) {
				if ( ! term_exists( $term['name'], 'alpha_people' ) ) {
					wp_insert_term( $term['name'], 'alpha_people', array( 'slug' => $term['code'] ) );
				}
			}
