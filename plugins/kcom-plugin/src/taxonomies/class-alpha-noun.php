...
class Alpha_Noun extends Taxonomies {
	...
	public function add_default_terms() {
		$taxonomy = get_terms( 'alpha_noun', array( 'hide_empty' => false ) );

		if ( empty( $taxonomy ) ) {
			$predefined_terms = $this->return_taxonomy_terms();

			foreach ( $predefined_terms as $term ) {
				if ( ! term_exists( $term['name'], 'alpha_noun' ) ) {
					wp_insert_term( $term['name'], 'alpha_noun', array( 'slug' => $term['code'] ) );
				}
			}
